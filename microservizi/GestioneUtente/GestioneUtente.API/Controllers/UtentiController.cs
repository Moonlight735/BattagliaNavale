...
public class UtentiController
{
    ...
    // POST: api/
    [HttpPost("{idStanza}/sposta-utenti-nella-stanza-padre")]
    public async Task<IActionResult> SpostaTuttiGliUtentiNellaStanzaPadre(int idStanza)
    {
        try
        {
            bool result = await _business.SpostaTuttiGliUtentiNellaStanzaPadre(idStanza);

            if (!result)
            {
                return new BadRequestObjectResult($"Non è stato possibile spostare gli utenti dalla stanza con ID {idStanza} alla stanza padre.");
            }

            return new OkObjectResult("Gli utenti sono stati spostati con successo nella stanza padre.");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"Errore durante lo spostamento degli utenti dalla stanza con ID {idStanza} alla stanza padre.");
            //return StatusCode(500, "Errore interno del server durante lo spostamento degli utenti.");
            return new BadRequestObjectResult($"Non è stato possibile spostare gli utenti dalla stanza con ID {idStanza} alla stanza padre.");
        }
    }
}
